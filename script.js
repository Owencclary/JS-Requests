...
sayHelloButton.addEventListener('mouseover', function() { changeBackgroundColor(sayHelloButton) })
...
// CODE HERE
function turnBlackgroundOff(elm) {
    elm.style.backgroundColor = '#EFEFEF'
}
sayHelloButton.addEventListener('mouseout', function() { turnBlackgroundOff(sayHelloButton) })
...
// CODE HERE
sayHelloButton.addEventListener('click', function() { sayHello } )
...
const ohMy = () => {
    ...
    axios.get('http://localhost:3000/animals').then(response => {
        for (let i; i < response.data.length; i++) {
            ...
            pElem.textContent = response.data[i]
            document.body.appendChild(pElem)
        }
    })
}

let animalsButton = document.getElementById('animals-button')
animalsButton.addEventListener('click', ohMy)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/string').then(response => response.data)
}
let repeatButton = document.getElementById('repeat-button')
repeatButton.addEventListener('click', function() { repeatMyParam } )
...
function sendQueryRequest() {
    axios.get('http://localhost:3000/query-test').then(({ data }) => console.log(data))

}
queryButton = document.getElementById('#query-button')

queryButton.addEventListener('click', function() { sendQueryRequest })
...
// PROBLEM 9
/* 
    Back in the ohMy function on Problem 5, replace the console log in the promise's callback with a for loop that 
    
    loops over res.data. 

    On each iteration of the loop, create a new p element. Set its textContent equal 
    
    the string at the current index (i) and then append the new p element onto the document's body. 
*/
...
//PROBLEM 11
/*
    You are going to add the ability to POST to the server. You'll need to create a small form and write a function that 
    
    makes a post request. Then you'll attach that function to the submit event on the form. We'll be creating a list of foods. 
...
foodForm.addEventListener('submit', function (event) {
    ...
    // Call the createFood function when the form is submitted
    createFood();
  });
...
axios.post('/food', body).then(res => {
    ...
    responseDataDiv.textContent = `Added food: ${response.data.newFood}`;
})